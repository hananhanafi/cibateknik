<template>
    <div class="bg-white shadow-sm rounded-8 p-3 text-left">
        ...
        <div class="form-check mr-2 form-control-sm">
            <input id="check-all" v-model="checkAll" class="form-check-input" type="checkbox" value="all" @change="checkAllHandler()">
            ...
            <div class="ml-auto mb-2 text-right">
                <button class="btn btn-success mr-2 mt-1" type="button" @click="$router.push({name:'admin-post-tambah-post-barang'})"><fa :icon="['fas','plus']"/> Barang</button>
            </div>
            <div class="ml-auto mb-2 text-right mt-1">
                <button :disabled="isEmptyCheckedItemList" type="button" title="Hapus Barang" class="btn btn-danger" @click.prevent="showAdminModalDeleteItemPosted"><fa :icon="['fas','trash']"/></button>
            </div>
        </div>
