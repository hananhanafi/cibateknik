<template>
    <div class="bg-white shadow-sm rounded-8 p-3 text-left">
        <div class="mb-3 p-2">
            <h1>Katalog Barang Rekomendasi</h1>
        </div>
        <div class="d-md-flex mb-3 px-2">
        <div class="form-check mr-2 form-control-sm">
            <input id="check-all-recommendation-item" v-model="checkAll" class="form-check-input" type="checkbox" value="all" @change="checkAllHandler()">
            <label class="form-check-label" for="check-all-recommendation-item">
                semua
            </label>
        </div>
            ...
            <div class="ml-auto mb-2 text-right">
                <button class="btn btn-success mr-2 mt-1" type="button" @click="showAdminModalListItemPostedAddRecommendationItem"><fa :icon="['fas','plus']"/> Rekomendasi</button>
            </div>
            <div class="ml-auto mb-2 text-right mt-1">
                <button :disabled="isEmptyCheckedItemList" type="button" title="Hapus Barang" class="btn btn-danger" @click.prevent="showAdminModalDeleteItemPostedRecommendation"><fa :icon="['fas','trash']"/></button>
            </div>
        </div>
        ...
        <AdminModalListItemPostedAddRecommendationItem :show="isShowAdminModalListItemPostedAddRecommendationItem" :options="options" @close="closeAdminModalListItemPostedAddRecommendationItem" @update="resetData"/>
        
        <AdminModalDeleteItemPostedRecommendation :show="isShowAdminModalDeleteItemPostedRecommendation" :data="checkedItemList" @close="closeAdminModalDeleteItemPostedRecommendation" @update="resetData"/>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            isShowAdminModalDeleteItemPostedRecommendation: false,
    ...
    computed: {
        ...
        methods: {
            ...
            showAdminModalListItemPostedAddRecommendationItem() {
                this.isShowAdminModalListItemPostedAddRecommendationItem = true;
            },
            closeAdminModalListItemPostedAddRecommendationItem() {
                this.isShowAdminModalListItemPostedAddRecommendationItem = false;
            },
            showAdminModalDeleteItemPostedRecommendation() {
                this.isShowAdminModalDeleteItemPostedRecommendation = true;
            },
            closeAdminModalDeleteItemPostedRecommendation() {
                this.isShowAdminModalDeleteItemPostedRecommendation = false;
            },
        },
}
</script>
