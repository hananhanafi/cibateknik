<template>
    <div class="bg-white shadow-sm rounded-8 p-3 text-left">
        <div class="mb-3 p-2">
            <h1>Katalog Barang Terbaru</h1>
        </div>
        <div class="d-md-flex mb-3 px-2">
        <div class="form-check mr-2 form-control-sm">
            <input id="check-all-newest-item" v-model="checkAll" class="form-check-input" type="checkbox" value="all" @change="checkAllHandler()">
            <label class="form-check-label" for="check-all-newest-item">
                semua
            </label>
        </div>
            ...
            <div class="ml-auto mb-2 text-right">
                <button class="btn btn-success mr-2 mt-1" type="button" @click="showAdminModalListItemPostedAddNewestItem"><fa :icon="['fas','plus']"/> Terbaru</button>
            </div>
            <div class="ml-auto mb-2 text-right mt-1">
                <button :disabled="isEmptyCheckedItemList" type="button" title="Hapus Barang" class="btn btn-danger" @click.prevent="showAdminModalDeleteItemPostedNewest"><fa :icon="['fas','trash']"/></button>
            </div>
        </div>
        ...
        <AdminModalListItemPostedAddNewestItem :show="isShowAdminModalListItemPostedAddNewestItem" :options="options" @close="closeAdminModalListItemPostedAddNewestItem" @update="resetData"/>
        
        <AdminModalDeleteItemPostedNewest :show="isShowAdminModalDeleteItemPostedNewest" :data="checkedItemList" @close="closeAdminModalDeleteItemPostedNewest" @update="resetData"/>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            isShowAdminModalDeleteItemPostedNewest: false,
    ...
    computed: {
        ...
        methods: {
            ...
            showAdminModalListItemPostedAddNewestItem() {
                this.isShowAdminModalListItemPostedAddNewestItem = true;
            },
            closeAdminModalListItemPostedAddNewestItem() {
                this.isShowAdminModalListItemPostedAddNewestItem = false;
            },
            showAdminModalDeleteItemPostedNewest() {
                this.isShowAdminModalDeleteItemPostedNewest = true;
            },
            closeAdminModalDeleteItemPostedNewest() {
                this.isShowAdminModalDeleteItemPostedNewest = false;
            },
        },
}
</script>
