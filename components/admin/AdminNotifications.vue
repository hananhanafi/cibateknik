<template>
    <div class="notif-container">
        <a class="btn notif-btn p-3" @click.prevent="toggleShowNotif">
            <fa class="text-20 text-white" :icon="['fas','bell']"/>
            ...
            transform: translateX(-50%);">
                ...
                <div class="container">
                    <div v-if="unReadNotificationCounter>0" class="row" style="max-height:300px;overflow-y:auto">
                        <div v-for="(notification,i) in adminNotifications" :key="i" class="col-12 ml-0 notif border-bottom border-top">
                ...
                <div class="p-3 text-center border-top">
                    ...
                    <router-link class="btn btn-sm" :to="{ name: 'admin-inbox' }">
                        Lihat Semua
                    </router-link>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async setReadAllNotification(){
            await ApiService.post(`/admin/notification/read`)
            .then(()=>{
        ...
        async loadNotif(){
            ...
            await ApiService.get(`/admin/unread-notifications`).then(data=>{
                this.adminNotifications = data.data;
                this.unReadNotificationCounter = this.adminNotifications.length;
            })
            .catch((err)=>{
                this.errors=err;
            })
